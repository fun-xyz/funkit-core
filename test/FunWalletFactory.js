...
const { HARDHAT_FORK_CHAIN_NAME, RPC_URL, PKEY, TEST_API_KEY } = require("./TestUtils")
...
describe("FunWalletFactory", function() {
    ...
    it("wallet should have the same address even we create the wallet twice", async function() {
        this.timeout(10000)
        const walletConfig = new FunWalletConfig(eoa, HARDHAT_FORK_CHAIN_NAME, 0.3)
        const wallet = new FunWallet(walletConfig, TEST_API_KEY)
        await wallet.init()

        const walletConfig1 = new FunWalletConfig(eoa, HARDHAT_FORK_CHAIN_NAME, 0)
