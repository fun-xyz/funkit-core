...
import { createUser, getUserUniqueId, getUserWalletIdentities, getUserWallets } from "../apis/UserApis"
...
export class NewAuth {
    ...
    async getAuthUniqueId(chainId: string): Promise<Hex> {
        await this.init()
        const storedAuthUniqueId = await getUserUniqueId(this.authId!)

        if (storedAuthUniqueId) {
            return storedAuthUniqueId as Hex
        } else {
            const words = this.authId!.split("###")
            let method
            ...
            const authUniqueId = keccak256(toBytes(uuidv4()))
            await createUser(this.authId!, chainId, addr, method, authUniqueId)
            return authUniqueId
        }
    }
