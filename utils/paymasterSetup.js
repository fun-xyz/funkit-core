...
const main = async () => {
    ...
    let startWalletPaymasterUSDC = (await paymasterSponsor.depositInfo("0x177716aBA5dC062e51d3d08FE2b00588c6a5e4a3")).tokenAmount
    let startFunderPaymasterUSDC = (await paymasterSponsor.depositInfo(signer.address)).tokenAmount
    let startFunderPaymasterETH = (await paymasterSponsor.depositInfo(signer.address)).sponsorAmount
    console.log(startWalletPaymasterUSDC, startFunderPaymasterUSDC, startFunderPaymasterETH)

    await paymasterSetup(signer)

    startWalletPaymasterUSDC = (await paymasterSponsor.depositInfo("0x177716aBA5dC062e51d3d08FE2b00588c6a5e4a3")).tokenAmount
    startFunderPaymasterUSDC = (await paymasterSponsor.depositInfo(signer.address)).tokenAmount
    startFunderPaymasterETH = (await paymasterSponsor.depositInfo(signer.address)).sponsorAmount
    console.log(startWalletPaymasterUSDC, startFunderPaymasterUSDC, startFunderPaymasterETH)
...
const paymasterSetup = async (signer) => {
    const paymasterSponsor = new PaymasterSponsor(signer)
    await paymasterSponsor.init()
    await paymasterSponsor.stakeEth(signer.address, ".2")
    await paymasterSponsor.addTokenDepositTo(walletAddress, "100")
