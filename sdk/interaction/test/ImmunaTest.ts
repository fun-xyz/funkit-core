...
const main = async () => {
    ...
    const getUnsignedERC20AuthTx = async (from: string, controllerAddress: string, erc20Addr: string, amount: number, type: string) => {
        if (type === 'aave') {
            ...
            let rawTransaction = {
                ...
                'to': erc20Addr,
                'value': 0x0,
                'data': myContract.methods.transfer(controllerAddr, amount).encodeABI(),
    ...
    const storeUserOp = (userOp: UserOperationStruct) => {
    ...
    const unsignedAuthTx = getUnsignedERC20AuthTx(from, "0x8626f6940E2eb28930eFb4CeF49B2d1F2C9C1199", erc20Addr, 1e18, 'aave')
    ...
    const signedAuthTx = getSignedERC20AuthTx(unsignedAuthTx, privateKey,31337)
